...
class FaceEngine:
    ...
    def process_frame(self):
           ...
           # --- LOGIC ĐIỀU KHIỂN & NHẪN THUẬT LÙI XE ---
            if self.current_mode == MODE_AUTO:
                if detected_status == "MASTER":
                    ...
                    elif w_box > (self.target_width * 0.6): 
        ...
        # Khai báo các cặp lệnh "kẻ thù" ngược chiều nhau
        opposite_cmds = {"W": "B", "B": "W", "L": "R", "R": "L"}
        
        if self.is_transitioning:
            # Đang trong thời gian bóp phanh xả quán tính
            if (current_time - self.transition_start_time) < self.transition_delay:
                command = "S" # Ép xuất lệnh Dừng
                cv2.putText(frame, "DANG XA QUAN TINH...", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
            else:
                # Đã nghỉ đủ thời gian -> Hết transition, cho phép xuất lệnh mới
                self.is_transitioning = False
                self.last_sent_cmd = raw_command
        else:
            # Kiểm tra xem có đang bị bẻ lái gắt không
            if opposite_cmds.get(self.last_sent_cmd) == raw_command:
                self.is_transitioning = True
                self.transition_start_time = current_time
                command = "S" # Lập tức đạp phanh cái đã
            else:
                # Nếu đổi lệnh êm thì cho qua luôn
                self.last_sent_cmd = raw_command

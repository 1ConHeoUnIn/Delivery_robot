...
class FaceEngine:
    def __init__(self, video_source):
        ...
        # --- THÔNG SỐ CALIB (LIVE TUNING) ---
        self.min_speed = 40     # Tốc độ quẹo và rà phanh
        self.max_speed = 80     # Tốc độ quẹo tối đa
        self.brake_ratio = 0.6  # Khoảng cách phanh

        # --- KHÂU I BÙ TẢI NẶNG ---
        self.cruise_speed = 80       # Tốc độ đi thẳng êm ái
        self.max_integral = 100.0    # Lực nhồi tối đa cho phép (+100)
        self.safe_distance = 0.4     # Ngưỡng bắt đầu nhồi ga
        self.integral_boost = 0.0    # Biến tích luỹ

        # --- BỘ ĐỆM CHỐNG GIẬT CẦU H ---
        self.last_sent_cmd = "S"
        self.is_transitioning = False
        self.transition_start_time = 0
        self.transition_delay = 0.15  # [FIX LAG] Đã giảm xuống 0.15s để bẻ lái lanh lẹ hơn

        # --- TỐI ƯU FPS (FRAME SKIPPING) ---
        self.frame_count = 0          # [FIX LAG] Đếm số khung hình
        self.last_hand_state = False  # [FIX LAG] Lưu trạng thái tay
    ...
    def process_frame(self):
        ...
        self.frame_count += 1 # [FIX LAG] Tăng biến đếm frame lên 1 mỗi vòng lặp
        ...
        if best_box:
                    ...
                    if self.is_learning:
                        ...
                        try:
                            ...
                            if confidence < 75:
                                ...
                                color = (0, 0, 255)
                                if self.master_track_id == best_track_id:
                                    self.master_track_id = -1 
            ...
            if self.current_mode == MODE_AUTO:
                if detected_status == "MASTER":
                    ...
                    # --- [FIX LAG] CHỈ CHẠY MEDIAPIPE 4 FRAME 1 LẦN ---
                    if master_roi.shape[0] > 0 and master_roi.shape[1] > 0:
                        if self.frame_count % 4 == 0: 
                            ...
                            self.last_hand_state = False # Reset trạng thái
                            
                            if hand_results.multi_hand_landmarks:
                                for hand_landmarks in hand_results.multi_hand_landmarks:
                                    # Vẽ điểm (Có thể comment lại phần vẽ để mượt hơn nữa)
                    ...
                    hand_raised = self.last_hand_state # Tái sử dụng trạng thái cũ cho các frame bị skip
                    
                    if hand_raised:
                        command = "B"
                        cv2.putText(frame, "NHAN THUAT - LUI XE!", (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
                    else:
                        current_ratio = w_box / self.target_width
                        if current_ratio > self.brake_ratio:
                            command = "S" 
                            cv2.putText(frame, "QUA GAN - DUNG!", (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        else:
                            error = abs(center_x - 0.5)
                            if error > 0.1:
                                speed = int(self.min_speed + ((error - 0.1) / 0.4) * (self.max_speed - self.min_speed))
                                speed = min(self.max_speed, max(self.min_speed, speed))
                                if center_x < 0.5:
                                    command = f"R{speed}"
                                else:
                                    command = f"L{speed}"
                                cv2.putText(frame, f"ARC-TURN: {command}", (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                            else:
                                if current_ratio < self.safe_distance:
                                    self.integral_boost += 1.5  
                                    self.integral_boost = min(self.integral_boost, self.max_integral)
                                else:
                                    if self.integral_boost > 0:
                                        self.integral_boost -= 3.0 
                                        self.integral_boost = max(0.0, self.integral_boost)
                                        
                                base_speed = int(self.cruise_speed + self.integral_boost)
                                command = f"W{base_speed}"
                                cv2.putText(frame, f"SOFT-BOOST: {command}", (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                else:
                    command = "S"

        if self.current_mode == MODE_STANDBY or detected_status == "NOBODY":
            command = "S"
            if self.current_mode == MODE_STANDBY: status_text = "CHE DO: DUNG YEN"
            color = (100, 100, 100)

        # Trả khâu I về 0 nếu xe phanh cứng hoặc lùi lại
        if command[0] in ["S", "B"]:
            self.integral_boost = 0.0
        ...
        # --- GIAO TIẾP DEBOUNCE (NÓI GIỌNG NÓI) ---
        ...
        # --- HUD CHUẨN ---
        cv2.putText(frame, f"Turn Speed (U/J): {self.min_speed}", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        cv2.putText(frame, f"Brake Dist (I/K): {round(self.brake_ratio, 2)}", (20, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

...
class FaceEngine:
    ...
    # --- LUỒNG NGHE (Debug kỹ hơn) ---
    def listen_thread(self):
        while self.listening:
            if self.voice.is_busy(): time.sleep(0.5); continue
            
            text = self.ears.listen()
            if not text: continue
            
            # Xử lý từ khóa (Thêm nhiều từ đồng nghĩa hơn)
    ...
    def process_frame(self):
                ...
                try:
                    ...
                    if confidence < 85:
                        if id == 1: 
                            ...
                            name_display = "MASTER"
                            self.last_seen_master_time = current_time # [MỚI] Nạp đầy niềm tin
                            color = (0, 255, 0)
                        else: 
        ...
        if self.current_mode == MODE_STANDBY:
            ...
            # [LOGIC MỚI] Kiểm tra thời gian lần cuối thấy Master
            time_since_seen = current_time - self.last_seen_master_time
            
            if time_since_seen < self.trust_duration: # Nếu mới thấy trong vòng 5s
                command = self.voice_move_cmd
                status_text = f"VOICE: {self.voice_move_cmd} (Tin cay: {round(5-time_since_seen,1)}s)"
                color = (255, 0, 255)
            else:
                command = "S"
                status_text = "MAT DAU MASTER! (DUNG AN TOAN)"
                color = (0, 0, 255)

        # --- LOGIC GIAO TIẾP ---
        if detected_status != "NOBODY":
            if detected_status != self.last_status:
                self.speak_status(detected_status)
                self.last_status = detected_status
                self.last_speak_time = current_time
            else:
                limit = 10.0 if detected_status == "STRANGER" else 43200.0

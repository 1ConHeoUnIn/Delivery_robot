...
class FaceEngine:
    def __init__(self, video_source):
        ...
        self.min_speed = 40     # Tốc độ rà phanh khởi điểm (Trục X)
        self.max_speed = 80     # Tốc độ quay tối đa (Trục X)
        self.brake_ratio = 0.6  # Tỉ lệ khung hình để phanh (Trục Z)
        
        # --- KHÂU P CHO TRỤC Z (ĐI THẲNG & LEO GỜ) ---
        self.cruise_speed = 80       # Tốc độ đi thẳng êm ái trên đường phẳng
        self.boost_speed = 150       # Tốc độ nhồi ga tối đa để búng qua gờ
        self.safe_distance = 0.4     # Nếu khung người nhỏ hơn 40% -> Đang ở xa -> Nhồi ga
    ...
    def process_frame(self):
                ...
                if detected_status == "MASTER":
                    ...
                    if hand_raised:
                        ...
                        if error > 0.1:
                            ...
                            # --- KHÂU P CHO TRỤC Z (ĐI THẲNG & AUTO-BOOST) ---
                            current_ratio = w_box / self.target_width
                            
                            if current_ratio > self.brake_ratio: 
                                command = "S" # Đứng quá gần thì phanh gấp
                                cv2.putText(frame, "QUA GAN - DUNG!", (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                            else:
                                if current_ratio < self.safe_distance:
                                    # Mục tiêu ở xa (bị kẹt gờ hoặc Sếp đi nhanh) -> Nhồi ga tuyến tính
                                    z_error = self.safe_distance - current_ratio
                                    max_z_error = self.safe_distance - 0.1 
                                    
                                    speed_w = int(self.cruise_speed + (z_error / max_z_error) * (self.boost_speed - self.cruise_speed))
                                    speed_w = min(self.boost_speed, max(self.cruise_speed, speed_w)) # Chốt chặn
                                    
                                    command = f"W{speed_w}"
                                    cv2.putText(frame, f"AUTO-BOOST: W{speed_w}", (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                                else:
                                    # Nằm trong vùng an toàn -> Chạy tốc độ êm ái
                                    command = f"W{self.cruise_speed}"
                else:
                    command = "S" 
        ...
        # --- HIỂN THỊ HUD CALIB ---
        cv2.putText(frame, f"Min Speed (U/J): {self.min_speed}", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        cv2.putText(frame, f"Brake Dist (I/K): {round(self.brake_ratio, 2)}", (20, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        cv2.putText(frame, f"Boost Speed (O/L): {self.boost_speed}", (20, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
